...
# Dernière édition : Noé, 16/03/2026, 18:45
...
def level_select() :
    ...
    surface.fill(DARK_GREY)
    print_txt("CHOISIS UN NIVEAU", (525, 100), 40, WHITE, True)
    ...
    for l in range(5):
        for i in range(10):
            ...
            if button((x_pos, y_pos, 60, 60), str(num), WHITE, 30, BLACK, YELLOW):
                current_level = num # On enregistre le niveau
                menu = "game"       
            num += 1
def game_screen():
    '''Affiche l'écran du niveau sélectionné (le labo gris)'''
    global menu
    surface.fill((60, 60, 60)) 
    
    
    print_txt(f"NIVEAU {current_level}", (525, 100), 50, WHITE, True)
